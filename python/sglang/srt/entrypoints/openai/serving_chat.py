...
class OpenAIServingChat(OpenAIServingBase):
    ...
    async def _handle_request_with_decode_retrieval(
        ...
        if request.tools or request.tool_choice != "none":
            return self.create_error_response(
                "Vector search decode-time injection is not compatible with tool calls yet.",
                err_type="NotImplementedError",
                status_code=400,
            )
        if request.logprobs:
            return self.create_error_response(
                "Vector search decode-time injection is not compatible with logprobs yet.",
                err_type="NotImplementedError",
                status_code=400,
            )
        ...
        total = request.max_tokens or request.max_completion_tokens
        if total is None or int(total) <= 0:
            return await super().handle_request(request, raw_request)
        remaining = int(total)

        base_messages = copy.deepcopy(request.messages)
        injected_messages: list[ChatCompletionMessageGenericParam] = []
        assistant_prefix = ""
        ...
        while remaining > 0:
            ...
            chunk_req.max_completion_tokens = None

            chunk_req.messages = copy.deepcopy(base_messages) + copy.deepcopy(
                injected_messages
            )
            if assistant_prefix:
                chunk_req.messages.append(
                    ChatCompletionMessageGenericParam(
                        role="assistant", content=assistant_prefix
                    )
                )
                chunk_req.continue_final_message = True
            else:
                chunk_req.continue_final_message = False
        ...
        combined_ret = {
            ...
            "meta_info": {
                "id": first_id or f"chatcmpl-{uuid.uuid4().hex}",
        ...
        return self._build_chat_response(request, [combined_ret], int(time.time()))
